...
if($help){
    ...
    Write-Host "Standard values:`n`t`tdest=.\source(copy)`n`t`tsource=.\`n`t`tfirst = ALL`n`t`tpattern = ALL`n`t`tpattern_exclude = false`n`t`tRobo1=\MIR`n`t`tRobo2=\copyal`n`t`tlRobo3-4=null"
